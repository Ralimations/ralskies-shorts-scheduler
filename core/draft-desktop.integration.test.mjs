...
test('desktop watcher, metadata form, stale reservation guard, calendar and stop control work through IPC on a disposable tracker',{skip:!available},async t=>{
  ...
  await poll();assert.equal(notifications.length,1);assert.equal(notifications[0].added,1);await assert.rejects(fs.access(video));
  let status=await call('draft-status');const row=status.rows[0];assert.equal(row.status,'AWAITING_METADATA');
  const input={batchId:row.batch_id,startDate:'2027-09-14',slots:['17:30'],cadence:'every-other-day'};
  const preview=await call('draft-reserve-preview',input);
  const snapshot={syncedAt:'2027-09-01T00:00:00Z',videos:[{id:'manual',snippet:{title:'Manual'},status:{privacyStatus:'private',publishAt:'2027-09-14T09:30:00Z'}}]};
  ...
  const fresh=await call('draft-reserve-preview',input);assert.equal(fresh.updates[0].scheduled_date,'2027-09-16');
  ...
  const calendar=await call('calendar');assert.ok(calendar.events.some(event=>event.shortId===row.short_id&&event.pht==='2027-09-16 17:30'&&event.state==='RESERVED'));
