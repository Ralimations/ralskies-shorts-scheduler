...
export function derivePrivateUploadOperations(batch,{channelId=''}={}){
  ...
  return batch.readiness.filter(item=>{const row=item.tracker||{};const intake=/^INTAKE-/.test(batch.batchId)&&['AWAITING_METADATA','BATCH_READY'].includes(upper(row.status));return item.classification==='MATCHED'&&(intake||item.metadata?.state==='METADATA_READY')&&item.productionEligibility==='AWAITING_PRIVATE_UPLOAD'&&(intake||upper(row.status)==='BATCH_READY')&&upper(row.schedule_eligible)==='YES'&&!/DUPLICATE|UNRESOLVED/i.test(clean(row.duplicate_disposition))&&!clean(row.youtube_video_id);}).map(item=>{
...
export async function runPrivateUploadPlan({plan,batch,approved=false,approvalPhrase='',adapter,readRemote,trackerUpdate=async()=>{},executionRecordPath,journalPath,now=()=>new Date().toISOString()}={}){
  ...
  for(const [index,op] of plan.operations.entries()){
    ...
    let inserted;try{inserted=await adapter.upload(op,{...context,saveSession:async url=>{journal.rows[index].resumableSessionUrl=url;journal.rows[index].updatedAt=now();await writeJson(journalPath,journal);}});}catch(error){journal.rows[index].state='REMOTE_STATE_UNKNOWN';journal.rows[index].error=String(error?.message||error);journal.rows[index].updatedAt=now();await writeJson(journalPath,journal);execution.state='RECOVERY_REQUIRED';execution.updatedAt=now();await writeJson(executionRecordPath,execution);throw error;}
