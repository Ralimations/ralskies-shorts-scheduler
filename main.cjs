...
ipcMain.handle('engine:bulk-metadata-audit',async(_e,batchId)=>{const {auditBulkMetadata}=await import('./core/bulk-metadata.mjs');return auditBulkMetadata({batchId,outputDir:OUT,trackerPath:TRACKER,transactionPath:BATCH_TRANSACTION});});
ipcMain.handle('engine:bulk-metadata-save',async(_e,p)=>{const {saveBulkMetadata}=await import('./core/bulk-metadata.mjs');return saveBulkMetadata({...p,outputDir:OUT,trackerPath:TRACKER,transactionPath:BATCH_TRANSACTION});});
...
ipcMain.handle('engine:settings',()=>fs.existsSync(SETTINGS)?JSON.parse(fs.readFileSync(SETTINGS,'utf8')):{timezone:'Asia/Manila',slots:['20:00'],cadence:'four-per-week',postsPerDay:1,horizonDays:14,optionalSlotEnabled:false,protectedWindow:'20:01–21:00',shortsPerDay:1,dryRun:true});
ipcMain.handle('engine:save-settings',async(_e,settings)=>{const {validateSlots,windowSlots}=await import('./core/schedule-calendar.mjs');if(settings.newDraftWindow){windowSlots({...settings.newDraftWindow,optionalSlot:settings.optionalSlotEnabled===true});if(!Number.isInteger(settings.newDraftWindow.postsPerDay)||settings.newDraftWindow.postsPerDay<1||settings.newDraftWindow.postsPerDay>10)throw Error('POSTS_PER_DAY_MUST_BE_1_TO_10');}if(settings.timezone!=='Asia/Manila')throw Error('TIMEZONE_MUST_BE_ASIA_MANILA');const saved={...settings,slots:validateSlots(settings.slots,settings.optionalSlotEnabled===true)};const {writeJson}=await import('./core/pipeline-store.mjs');await writeJson(SETTINGS,saved);return saved});
