...
test('five-screen navigation, saved reservation defaults, and historical log pagination work',async()=>{
 ...
 const settings={timezone:'Asia/Manila',slots:['20:00'],cadence:'four-per-week',optionalSlotEnabled:false};
 ...
 w.document.querySelector('[data-draft-schedule]').click();assert.equal(w.document.querySelector('#draft-window-start').value,'20:00');assert.equal(w.document.querySelector('#draft-schedule-batch').value,'INTAKE-ALL');assert.equal(w.document.querySelector('#draft-window-end'),null);assert.match(w.document.body.textContent,/next 14 days/);
