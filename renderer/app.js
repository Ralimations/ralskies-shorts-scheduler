...
function settings(){
 const s=state.settings||{},w=s.newDraftWindow||{windowStart:'20:00',windowEnd:'02:00',intervalMinutes:30,postsPerDay:1};
 return '<section class="panel settings"><h2>Scheduling defaults</h2><p>Used when you open a new reservation review. Existing schedules stay as they are.</p><label>Timezone<input id="setting-timezone" value="Asia/Manila" readonly></label><label>Posting times (comma separated)<input id="setting-slots" value="'+esc((s.slots||['17:30','22:30']).join(', '))+'"></label><label><input id="setting-optional" type="checkbox" '+(s.optionalSlotEnabled?'checked':'')+'> Allow optional 01:30 slot</label><p class="subtle">20:00-21:00 is reserved for manual posts.</p><h3>New draft window</h3><label>Starts<input type="time" id="setting-window-start" value="'+esc(w.windowStart)+'"></label><label>Ends (may be next day)<input type="time" id="setting-window-end" value="'+esc(w.windowEnd)+'"></label><label>Candidate interval (minutes)<input type="number" id="setting-window-interval" min="15" value="'+w.intervalMinutes+'"></label><label>Shorts per night<input type="number" id="setting-window-count" min="1" max="10" value="'+w.postsPerDay+'"></label><button class="primary" data-save-settings>Save Defaults</button></section><section class="panel section"><h2>Connections and intake</h2><p>Configure your existing local model or choose where new videos arrive.</p><div class="button-row"><button class="ghost" data-analytics-open>Weekly Analytics</button><button class="ghost" data-llm-settings>LM Studio Connection</button><button class="ghost" data-draft-folder>Choose Draft Folder</button></div><p class="subtle">YouTube authorization uses the existing OAuth setup. Changing these settings does not publish videos.</p></section><details class="panel section"><summary>Legacy title reviews ('+(state.titleQueue?.length||0)+')</summary>'+titleLab()+'</details>';
}
function logs(){
 const activity=state.activity||{entries:[],total:0},page=state.activityPage||0,rows=activity.entries.slice(page*100,(page+1)*100);
 return '<section class="panel"><div class="section-title"><h2>Needs attention</h2><span class="pill">'+state.recovery.length+' saved recovery records</span></div><p class="subtle">Recorded state from disk; this is not a fresh YouTube check.</p>'+(state.recovery.length?state.recovery.map(row=>'<div class="callout"><strong>'+esc(row.short_id)+' - '+esc(row.state)+'</strong><span>Execution '+esc(row.execution_id)+' / YouTube '+esc(row.youtube_video_id||'unknown')+'</span></div>').join(''):'<div class="empty">No unresolved production recovery items.</div>')+'</section><section class="panel section"><div class="section-title"><h2>Activity history</h2><span class="pill">'+activity.total+' saved entries</span></div><p class="subtle">All retained execution logs, journal snapshots, and exceptions, newest first. Older events that were never recorded cannot be reconstructed.</p><div class="button-row"><button class="ghost" data-history-page="-1" '+(page===0?'disabled':'')+'>Newer</button><span>Page '+(page+1)+' of '+Math.max(1,Math.ceil(activity.total/100))+'</span><button class="ghost" data-history-page="1" '+((page+1)*100>=activity.total?'disabled':'')+'>Older</button></div>'+(rows.length?'<table><thead><tr><th>Time</th><th>Event / state</th><th>Video / batch</th><th>Details</th></tr></thead><tbody>'+rows.map(row=>'<tr><td>'+esc(row.timestamp)+'</td><td>'+esc(row.event)+'<br>'+esc(row.state)+'</td><td>'+esc(row.shortId)+'<br>'+esc(row.batchId)+'</td><td>'+esc(row.message)+'<details><summary>Source</summary>'+esc(row.file)+'<br>'+esc(row.executionId)+'</details></td></tr>').join('')+'</tbody></table>':'<div class="empty">No saved activity found.</div>')+'</section>';
}
...
async function refresh(){
  const [inventory,calendar,batches,titleQueue,recovery,settings]=await Promise.all([Promise.resolve(null),window.ralskies.calendar(),window.ralskies.batches(),window.ralskies.titleReviewQueue(),window.ralskies.recoveryJournals(),window.ralskies.settings()]);
...
async function selectBatch(batchId){state.view='batches';state.selectedBatch=batchId;state.batchDetail=await window.ralskies.batchDetail(batchId);if(historyBatch({batchId,detail:state.batchDetail}))state.showUploadHistory=true;state.modal=null;render();notify(`${batchId} selected. ${workflow().title}.`);}
...
document.addEventListener('click',async event=>{
  ...
  try{
    if(target.closest('button')?.getAttributeNames().some(name=>name.startsWith('data-analytics-'))){await window.AnalyticsView.action(target);return;}
    ...
    if(target.closest('[data-save-settings]')){state.settings=await window.ralskies.saveSettings({...(state.settings||{}),timezone:$('#setting-timezone').value,slots:$('#setting-slots').value.split(',').map(value=>value.trim()).filter(Boolean),optionalSlotEnabled:$('#setting-optional').checked,newDraftWindow:{windowStart:$('#setting-window-start').value,windowEnd:$('#setting-window-end').value,intervalMinutes:Number($('#setting-window-interval').value),postsPerDay:Number($('#setting-window-count').value)}});notify('Settings saved.');return;}
...
function showDraftSchedule(){
 const batches=[...new Set((state.drafts?.rows||[]).filter(row=>['AWAITING_METADATA','BATCH_READY','PRIVATE_UPLOADED'].includes(row.status)).map(row=>row.batch_id))];
 if(!batches.length){notify('Intake new drafts first.','info');return;}
 const w=state.settings?.newDraftWindow||{windowStart:'20:00',windowEnd:'02:00',intervalMinutes:30,postsPerDay:1},today=new Date(Date.now()+8*3600000).toISOString().slice(0,10);
 setModal('Validate new release window','<p>Check the live calendar, then reserve dates before generating metadata. The window may cross midnight; early-morning times belong to the following day.</p><label>New batch<select id="draft-schedule-batch">'+batches.map(id=>'<option value="'+esc(id)+'">'+esc(id)+'</option>').join('')+'</select></label><label>Starting night (Manila)<input type="date" id="draft-start" min="'+today+'" value="'+today+'"></label><label>Window starts<input type="time" id="draft-window-start" value="'+esc(w.windowStart)+'"></label><label>Window ends<input type="time" id="draft-window-end" value="'+esc(w.windowEnd)+'"></label><label>Candidate interval (minutes)<input type="number" min="15" max="1440" id="draft-interval" value="'+w.intervalMinutes+'"></label><label>Shorts per night<input type="number" min="1" max="10" id="draft-per-day" value="'+w.postsPerDay+'"></label><label>Frequency<select id="draft-cadence"><option value="daily">Daily</option><option value="every-other-day">Every other day</option></select></label><label class="checkbox-label"><input id="draft-optional" type="checkbox" '+(state.settings?.optionalSlotEnabled?'checked':'')+'> Enable optional 01:30 slot</label><p>20:00-21:00 stays manual-only. Occupied and past times are skipped. Existing reservations remain unchanged.</p>','<button class="ghost" data-close-modal>Cancel</button><button class="primary" data-draft-reservation-preview>Check Calendar &amp; Validate</button>');
}
async function draftAction(target){
  ...
  if(target.closest('[data-draft-reservation-preview]')){
    const plan=await window.ralskies.draftReservePreview({batchId:$('#draft-schedule-batch').value,startDate:$('#draft-start').value,windowStart:$('#draft-window-start').value,windowEnd:$('#draft-window-end').value,intervalMinutes:Number($('#draft-interval').value),postsPerDay:Number($('#draft-per-day').value),cadence:$('#draft-cadence').value,optionalSlot:$('#draft-optional').checked});
    state.reservationPlan=plan;
    setModal('Schedule checks passed',`<p>${plan.updates.length} new reservations. YouTube sync: ${esc(plan.snapshotAt||'not yet synced; tracker only')}.</p><table><thead><tr><th>Short</th><th>Date</th><th>Time (Manila)</th></tr></thead><tbody>${plan.updates.map(row=>`<tr><td>${esc(row.short_id)}</td><td>${esc(row.scheduled_date)}</td><td>${esc(row.scheduled_time)}</td></tr>`).join('')}</tbody></table>`,'<button class="ghost" data-close-modal>Cancel</button><button class="primary" data-draft-reservation-save '+(plan.updates.length?'':'disabled')+'>Greenlight Schedule &amp; Save Reservations</button>');return true;
  }
  if(target.closest('[data-draft-reservation-save]')){
    await window.ralskies.draftReserve({plan:state.reservationPlan});state.modal=null;await refreshDrafts();notify('Schedule greenlit locally. You can now generate and review metadata.');return true;
  }
...
function lmConnectionInput(){
  ...
  return {baseUrl:$('#lm-protocol').value+'://'+host+':'+port+'/v1',model:$('#lm-model').value.trim(),timeoutSeconds:Number($('#lm-timeout').value),apiMode:$('#lm-api-mode').value};
}
async function showLmSettings(){
  const config=await window.ralskies.metadataSettings(),url=new URL(config.baseUrl);
  setModal('LM Studio connection',`<p>Connect to the model you already serve in LM Studio.</p><label>Local server address<input id="lm-host" value="${esc(url.hostname)}" placeholder="127.0.0.1"></label><label>Port<input id="lm-port" type="number" min="1" max="65535" value="${esc(url.port||'1234')}"></label><label>Protocol<select id="lm-protocol"><option value="http" ${url.protocol==='http:'?'selected':''}>HTTP</option><option value="https" ${url.protocol==='https:'?'selected':''}>HTTPS</option></select></label><label>Model ID (optional if the server exposes one model)<input id="lm-model" value="${esc(config.model)}" placeholder="Use the model already served by LM Studio"></label><label>API mode<select id="lm-api-mode"><option value="native" ${config.apiMode==='native'?'selected':''}>LM Studio native (reasoning off)</option><option value="openai" ${config.apiMode!=='native'?'selected':''}>OpenAI-compatible</option></select></label><label>Generation timeout (seconds)<input id="lm-timeout" type="number" min="30" max="600" value="${esc(config.timeoutSeconds||300)}"></label><div id="lm-connection-status" class="subtle" role="status">In LM Studio, enable the server in the Developer tab.</div>`,'<button class="ghost" data-close-modal>Close</button><button class="ghost" data-llm-test>Test Connection</button><button class="primary" data-llm-save-settings>Save Connection</button>');
}
