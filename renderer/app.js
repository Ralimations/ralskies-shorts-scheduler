...
function settings(){
 ...
 return '<section class="panel settings"><h2>Publishing plan</h2><p>Four posts per week: Monday, Wednesday, Friday, Sunday. One post per day, with at least seven days between clips of the same song.</p><p>Plan only the next two weeks. New drafts from every intake batch can fill open dates. Mark your own songs as originals to prioritize them.</p><label>Timezone<input id="setting-timezone" value="Asia/Manila" readonly></label><label>Posting time<input type="time" id="setting-slots" value="'+esc(s.slots?.[0]||'20:00')+'"></label><p class="subtle">20:01–21:00 is reserved for manual posts. Existing reservations keep their saved times.</p><button class="primary" data-save-settings>Save Defaults</button></section><section class="panel section"><h2>Connections and intake</h2><div class="button-row"><button class="ghost" data-analytics-open>Weekly Analytics</button><button class="ghost" data-llm-settings>LM Studio Connection</button><button class="ghost" data-draft-folder>Choose Draft Folder</button></div><p class="subtle">Changing settings does not upload or publish videos.</p></section><details class="panel section"><summary>Legacy title reviews ('+(state.titleQueue?.length||0)+')</summary>'+titleLab()+'</details>';
}
function logs(){
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(target.closest('[data-save-settings]')){state.settings=await window.ralskies.saveSettings({...(state.settings||{}),timezone:$('#setting-timezone').value,slots:[$('#setting-slots').value],cadence:'four-per-week',postsPerDay:1,shortsPerDay:1,horizonDays:14,optionalSlotEnabled:false,newDraftWindow:{windowStart:$('#setting-slots').value,windowEnd:$('#setting-slots').value,intervalMinutes:30,postsPerDay:1}});notify('Settings saved.');return;}
...
function showDraftSchedule(){
 ...
 setModal('Plan the next two weeks','<p>Up to four posts per week: Monday, Wednesday, Friday, Sunday. One post per day, with seven days between clips of the same song. Originals get first choice of open dates. New arrivals can fill later previews.</p><label>Drafts<select id="draft-schedule-batch"><option value="INTAKE-ALL">All new draft batches</option>'+batches.map(id=>'<option value="'+esc(id)+'">'+esc(id)+'</option>').join('')+'</select></label><label>Start date (Manila)<input type="date" id="draft-start" min="'+today+'" value="'+today+'"></label><label>Posting time<input type="time" id="draft-window-start" value="'+esc(state.settings?.slots?.[0]||'20:00')+'"></label><p>Only the next 14 days are reserved. Add the song name in draft metadata first. Existing reservations stay in place; unused dates remain empty. The 20:01–21:00 window stays manual-only.</p>','<button class="ghost" data-close-modal>Cancel</button><button class="primary" data-draft-reservation-preview>Check Calendar &amp; Preview</button>');
}
async function draftAction(target){
  ...
  if(edit){
    ...
    setModal('Draft metadata',`<p>${esc(row.original_filename)} · ${esc(row.short_id)}</p>${[['public_title','Title'],['source_song','Song'],['artist_or_fandom','Artist / fandom'],['youtube_tags','Tags (comma separated)'],['related_video_id','Related video ID (optional)']].map(([key,label])=>`<label>${label}<input id="draft-meta-${key}" value="${esc(row[key])}" ${key==='public_title'?'maxlength="100"':''}></label>`).join('')}<label>Performance type<select id="draft-meta-content_type"><option value="COVER" ${row.content_type!=='ORIGINAL'?'selected':''}>Cover</option><option value="ORIGINAL" ${row.content_type==='ORIGINAL'?'selected':''}>My original song</option></select></label><label>Description<textarea id="draft-meta-description" rows="6" maxlength="5000">${esc(row.description)}</textarea></label><label>Category<select id="draft-meta-category"><option value="">Choose category</option><option value="Music" ${row.category==='Music'?'selected':''}>Music</option></select></label><p>Blank required fields keep this draft waiting for metadata. Changes apply to this draft only.</p>`,'<button class="ghost" data-close-modal>Cancel</button><button class="primary" data-draft-metadata-save>Save Metadata</button>');return true;
  }
  if(target.closest('[data-draft-metadata-save]')){
    const metadata=Object.fromEntries(['content_type','public_title','description','source_song','artist_or_fandom','youtube_tags','related_video_id','category'].map(key=>[key,$('#draft-meta-'+key).value]));
  ...
  if(target.closest('[data-draft-reservation-preview]')){
    const plan=await window.ralskies.draftReservePreview({batchId:$('#draft-schedule-batch').value,startDate:$('#draft-start').value,slots:[$('#draft-window-start').value],postsPerDay:1,cadence:'four-per-week',horizonDays:14,optionalSlot:false});
    state.reservationPlan=plan;
    setModal('Schedule checks passed',`<p>${plan.updates.length} new reservations. ${plan.deferred?.length||0} drafts remain waiting for a later window or song identity. YouTube sync: ${esc(plan.snapshotAt||'not yet synced; tracker only')}.</p><table><thead><tr><th>Short</th><th>Date</th><th>Time (Manila)</th></tr></thead><tbody>${plan.updates.map(row=>`<tr><td>${esc(row.short_id)}</td><td>${esc(row.scheduled_date)}</td><td>${esc(row.scheduled_time)}</td></tr>`).join('')}</tbody></table>`,'<button class="ghost" data-close-modal>Cancel</button><button class="primary" data-draft-reservation-save '+(plan.updates.length?'':'disabled')+'>Greenlight Schedule &amp; Save Reservations</button>');return true;
  }
  if(target.closest('[data-draft-reservation-save]')){
    await window.ralskies.draftReserve({plan:state.reservationPlan});state.modal=null;await refreshDrafts();notify('Two-week plan saved locally. Add new drafts and preview again to fill open dates.');return true;
  }
