...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(target.closest('[data-save-settings]')){state.settings=await window.ralskies.saveSettings({...(state.settings||{}),timezone:$('#setting-timezone').value,slots:$('#setting-slots').value.split(',').map(value=>value.trim()).filter(Boolean),optionalSlotEnabled:$('#setting-optional').checked});notify('Settings saved.');return;}
